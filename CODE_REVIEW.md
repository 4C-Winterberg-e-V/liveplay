...
Diese Klarstellung ändert die Bewertung erheblich. Die ursprüngliche Einstufung ging vom „worst case" aus (feindlicher Akteur im selben Netz / offenes Internet). Da **beide** Zugriffswege bereits authentifiziert sind (CF Access remote, kontrolliertes Netz lokal), werden alle Befunde, die **einen Angreifer mit Netzwerk- oder Dateizugriff voraussetzen, herabgestuft** und als *Defense-in-Depth* geführt. Befunde zu **Zuverlässigkeit, Korrektheit, Datenintegrität und Wartbarkeit** bleiben unverändert — denn das eigentliche Risiko bei einem kleinen Event ist **ein Absturz oder Datenverlust mitten in der Veranstaltung**, nicht ein Angriff.

**Zwei Voraussetzungen, die diese Entlastung tragen** (bitte gegenprüfen):
1. **Cloudflare Access ist die echte Schutzschicht des Remote-Pfads.** Die Telefonsteuerung übers Internet ist nur akzeptabel, solange der Tunnel-Origin **ausschließlich** hinter CF Access erreichbar ist (keine zusätzliche ungeschützte öffentliche Route) und die CF-Access-Policy auf bekannte Personen beschränkt ist. Die app-eigene 4-stellige PIN (H-06) ist dann nur eine schwache **zweite** Schicht hinter CF Access — daher herabgestuft, aber langfristig trotzdem zu härten, falls CF Access je wegfällt.
2. **Das lokale Netz ist tatsächlich kontrolliert.** Für die LAN-/Server-Client-Betriebsart ist der `0.0.0.0`-Bind **erforderlich** — die Schutzschicht ist hier das abgeschottete Netz. Da der C++-Server **keine App-Authentifizierung** hat, hängt diese Betriebsart vollständig an der Netzgrenze; eine **optionale Token-Authentifizierung** wäre die sauberste Defense-in-Depth (ein Loopback-Bind ist hier gerade **nicht** möglich, sonst kann sich kein Client verbinden).
...
|---|---|---|---|
| **C-01** unauth. Control-Plane `0.0.0.0` | 🔴 Critical | 🟡 **Medium** | Remote durch **CF Access** gegated, lokal durch das kontrollierte Netz. `0.0.0.0` ist in der Server-/Remote-Client-Betriebsart **erforderlich** (kein Loopback möglich). Empfehlung daher: optionales **App-Token** als Defense-in-Depth, nicht Loopback. |
...
| **H-05** DNS-Rebinding (Proxy) | 🟠 High | ⚪ **Low** | Remote-Pfad liegt hinter CF Access; Origin/Host-Allowlist trotzdem sinnvoll. |
| **H-06** 4-stellige PIN ohne Rate-Limit | 🟠 High | ⚪ **Low** | Hinter CF Access nur schwache zweite Schicht; härten, falls CF Access je entfällt. |
| **H-07** Auth-Gate abschaltbar | 🟠 High | ⚪ **Low** | Selbst bei deaktivierter App-PIN bleibt CF Access davor; daher gering. |
