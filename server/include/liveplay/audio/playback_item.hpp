...
private:
    ...
    // No render-thread cache of the applied offset is needed: render_block()
    // pushes the offset through LTCGenerator::set_offset(), which never resets
    // the encoder, so there is nothing to debounce (H-16).
