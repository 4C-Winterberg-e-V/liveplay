...
bool ProjectState::save(const std::filesystem::path& path) const {
    ...
    try {
        ...
        // Serialise BEFORE touching the temp file, so a dump() throw (bad UTF-8
        // in a tag, etc.) never leaves a stray empty .tmp behind.
        ...
        // the same volume on POSIX and on modern Windows). A write error,
        // disk-full, or crash therefore never truncates or corrupts the previous
        // good file — the documented "preserve previous state on failure"
        // contract — and a failed write is reported as a real failure rather
        // than a false "saved". (fork M-17/H-15 + upstream #5)
        ...
        tmp += ".tmp";
        {
            ...
            f << serialized;
            f.flush();
            // Check AFTER close(), not before: close() performs the final flush
            // to the OS, so a disk-full error can surface there and nowhere else.
            f.close();
            if (!f.good()) {
                Logger::error("ProjectState::save: write to temp '{}' failed "
                              "(disk full / I/O error) — previous file left untouched",
                              util::path_to_utf8(tmp));
        ...
        if (ec) {
            Logger::error("ProjectState::save: rename '{}' -> '{}' failed: {} — "
                          ...
                          ec.message());
            // Drop the temp rather than leaving an orphan .tmp next to the
            // project: the document is still live in memory, so a retry loses
            // nothing, and nothing else ever cleans these up.
            std::error_code rm_ec;
            std::filesystem::remove(tmp, rm_ec);
            return false;
        }
...
bool ProjectState::play_item(const std::string& uuid,
  ...
  try {
    ...
    if (!device_override.empty()) {
        ...
        //
        // Upstream (#30) re-read settings.defaultOutputDevice here, because in
        // its tree this `else` was also the default-device path. In this fork
        // the branch above already routes to the project default device AND
        // honours the per-project hardware channel selection, so by the time we
        // get here default_device is empty by construction — repeating the
        // lookup would be dead code. Only the "drop stale routes" part is kept,
        // via upstream's unroute_item_from_all_mixers() helper (same effect as
        // this fork's manual per-source-channel unroute loop, less code).
...
ProjectState::ensure_device_routing(const std::string& device_name,
    ...
    audio::MasterChannelIndex master_r = 0;
    {
        ...
        // Master allocation is bound-checked inside claim_master_pair_locked():
        // it walks [kFirstDeviceMaster, min(kDeviceMasterLimit, live bus width))
        // so it can neither run past the engine's master bus nor collide with
        // the preview reserve. That covers upstream's #5 concern about trusting
        // the 30/31 literals, and unlike upstream's `next_override_master_ += 2`
        // bump allocator it RECYCLES pairs freed by release_master_pair_locked()
        // — a show that re-picks output devices repeatedly would otherwise leak
        // the bus dry.
    ...
    // Hardware channel choice is this fork's (channels.left/right instead of
    // upstream's hardcoded 0/1); the explicit lane argument is upstream's fix
    // for the strip being downmixed to mono on its way to the master bus.
    engine_.assign_master_to_device(master_l, dev.id, channels.left);
    if (!channels.mono()) {
        // Stereo selection: strip lane L → left hw channel, lane R → right.
        ...
        // Mono selection: one hardware channel, so both lanes are summed into
        // it (kAllMixerLanes) rather than dropping the right lane on the floor.
        engine_.route_mixer_to_master(mixer, master_l, 0.0f, audio::kAllMixerLanes);
    }
...
bool ProjectState::start_preview(const std::string& item_uuid) {
    ...
    audio::MixerChannelId preview_mixer;
    {
        ...
        if (!have_infra) {
            ...
            // Same synthesis as ensure_device_routing(): this fork's hardware
            // channel choice, upstream's explicit strip lane so the preview bus
            // isn't downmixed to mono on the way to the master.
            ...
            if (!preview_channels.mono()) {
                ...
                // A mono selection uses one master only, so the signal isn't
                // summed onto the same hardware channel twice — both lanes go
                // into that single master instead.
                engine_.route_mixer_to_master(mixer, kPreviewMasterL, 0.0f,
                                              audio::kAllMixerLanes);
            }
            {
                std::lock_guard lock{mutex_};
