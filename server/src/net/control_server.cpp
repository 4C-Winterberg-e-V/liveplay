        ...
        ([this](const crow::request& req){
            try {
                ...
                if (path.empty()) {
                    ...
                    // The filesystem sandbox is the single authority on what may be
                    // opened, so the "places" list is exactly its roots: listing
                    // anything else (upstream enumerated "/", /Volumes, /media, /mnt
                    // and every Windows drive letter for #31) would offer places the
                    // sandbox check then refuses.
                    //
                    // That means upstream's #31 goal — reaching a USB stick or other
                    // mounted volume — is NOT served out of the box here, by design:
                    // auto-adding every mount would widen the sandbox this fork
                    // deliberately introduced, and that is a call for the operator,
                    // not a default. The opt-in is the LIVEPLAY_FS_ROOTS env var
                    // (see FsSandbox::build_default_roots), which adds any path —
                    // a mounted stick, a NAS share, a sample drive — as a real root
                    // that then shows up in this list.
