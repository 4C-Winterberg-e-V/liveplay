...
std::size_t PlaybackItem::render_block(Sample* const* out_channel_buffers,
    ...
    if (ltc_on && ltc_ && out_channel_count > file_channels_) {
        ...
        // biphase-mark signal, H-16). Sample-rate / frame-rate / enable changes
        // go through configure() from their dedicated setters instead.
        //
        // This fork fixed the same bug by skipping configure() while the offset
        // was unchanged; upstream's set_offset() supersedes it, because an offset
        // change no longer forces a polarity reset either — render_block()
        // resyncs naturally on the block where the frame number changes.
        ltc_->set_offset(
